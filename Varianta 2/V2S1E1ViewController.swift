...
//
var scorv2s1 = 0
let initialdistance = CGAffineTransform(translationX: 500, y: 0)
let negativeinitialdistance = CGAffineTransform(translationX: 0, y: 0)

let transitiondistance = CGAffineTransform(translationX: -500, y: 0)

import UIKit

class V2S1E1ViewController: UIViewController, UIPopoverPresentationControllerDelegate {
    ...
    @objc func screenEdgeSwiped(_ recognizer: UIScreenEdgePanGestureRecognizer) {
        if recognizer.state == .recognized {
            performSegue(withIdentifier: "yousureaboutdat5", sender: self) //cell? sau ceil
        }
    }
